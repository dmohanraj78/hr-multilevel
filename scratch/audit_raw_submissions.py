...
def audit():
    ...
    # Fetch all records with a large limit
    ...
    # We will group rows that share either normalized email, phone (last 10 digits), github username, or exact full name
    ...
    for row in rows:
        ...
        for leader_id, group_rows in groups.items():
            for gr in group_rows:
                ...
                # Matching conditions
                if (email and email == ge) or \
                   (phone and len(phone) >= 10 and phone == gp) or \
                   (github and len(github) > 3 and github == gg) or \
                   (name and name == gn):
